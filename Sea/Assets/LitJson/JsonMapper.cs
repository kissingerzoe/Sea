...
namespace NbaLitJson
{
	...
	public class JsonMapper
	{
		...
		private static int static_list_deep = 0;
		private static bool static_list_dirty = true;
		...
		private static object ReadValue (Type inst_type, JsonReader reader)
		{
			...
			if(inst_type==null)
			{
				UnityEngine.Debug.LogWarning("here"+ reader.Token);
			}
			...
			if (reader.Token == JsonToken.Null) {
				if(inst_type==typeof(System.Int64)||inst_type==typeof(System.Int32) || inst_type==typeof(System.Double) || inst_type==typeof(System.Single))
				{	
					return 0;
				}
			...
			if (reader.Token == JsonToken.Double ||
				...
				if(inst_type==typeof(System.Int64))
				{
					...
					{
						return (System.Int32)reader.Value;
					}					
				}
				*/
				...
				{
					if(json_type==typeof(System.Int64))
					{
						//NGUIDebug.Log(">>>>>>>>>:"+reader.Value);
						return (System.Int32)reader.Value;
					}
					else if(json_type==typeof(System.String))
					{
						int result = 0;
				...
				{
					if(json_type==typeof(System.Double))
					{
						return (System.Single)reader.Value;
					}					
				}
				...
				//else if(typeof(IEnumerable).IsAssignableFrom(inst_type))
				//{
				//	return null;
				//}
				return null;
...
//                    reader.Value, json_type, inst_type));
			...
			if (reader.Token == JsonToken.ArrayStart) {
				...
				try {
					while (true) {
						static_list_dirty = true;
						object item = ReadValue (elem_type, reader);
						if (reader.Token == JsonToken.ArrayEnd) {
							if (static_list_dirty) {
								static_list_deep--;
								if (static_list_deep <= 0) {
									static_list_dirty = true;
								} else {
									static_list_dirty = false;
								}
								break;
							}
						}
						
						if(elem_type!=item.GetType())
						{
							UnityEngine.Debug.Log(item.GetType()+"  "+elem_type);
					...
					throw new JsonException (e.Message + ", row :" + curRow, e); 
				}
